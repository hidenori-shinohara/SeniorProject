...
In the previous chapter, we have introduced some matroids that are not representable over some field.
For example, we have proved that $U_{2, 4}$ is not representable over $GF(2)$.
Now, consider a matroid $M$ that is identical to $U_{2, 4}$ except that it has one extra element that is a loop.
It is not so hard to see that $M$ is not representable over $GF(2)$.
(We will mathematically prove this later.)
We can add two loops to $U_{2, 4}$ to get a matroid that is not representable over $GF(2)$.
Actually, we can add any number of loops to $U_{2, 4}$ and get a matroid that is not representable over $GF(2)$.
However, this is not a meaningful discovery since the structure that makes them unrepresentable over $GF(2)$ is $U_{2, 4}$.
In this sense, $U_{2, 4}$ is a structure that a representable matroid cannot ``contain".
In order to discuss this more mathematically, this chapter will introduce a new concept, a matroid minor.
